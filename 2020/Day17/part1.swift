...
for i in 0..<inputs.count {
	...
	for j in 0..<row.count {
		let position = threeDee(x: j, y: i, z: 0)
	...
	for calculateX in minX...maxX {
		for calculateY in minY...maxY {
			for calculateZ in minZ...maxZ {
				...
				let calculatePoint = threeDee(x: calculateX, y: calculateY, z: calculateZ)
				...
				for x in -1...1 {
					for y in -1...1 {
						for z in -1...1 {
							if !(x == 0 && y == 0 && z == 0) {
								let searchLocation = threeDee(x: calculateX + x, y: calculateY + y, z: calculateZ + z)
